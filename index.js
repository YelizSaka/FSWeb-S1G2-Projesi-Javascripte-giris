...
let no = "1999";
let yeniNo = Number(no);
console.log(yeniNo);
console.log(typeof(yeniNo)); 
...
function sesliHarfSayaci(kelime) {
  let newKelime = kelime.toLowerCase();
  let kelimeDizi = Array.from(newKelime);
  let sesliHarfSayisi = 0;
  for(i=0; i<kelimeDizi.length; i++){
    (kelimeDizi[i] === "e" || kelimeDizi[i] === "a" || kelimeDizi[i] === "o" || kelimeDizi[i] === "u" || kelimeDizi[i] === "i" || kelimeDizi[i] === "ö" || kelimeDizi[i] === "ü" || kelimeDizi[i] === "ı") ? sesliHarfSayisi++ : sesliHarfSayisi;
    }
